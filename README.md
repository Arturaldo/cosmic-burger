...
Учебный проект «Космическая бургерная» (проектная работа 1-го спринта): конструктор бургера на React с получением ингредиентов с сервера и модальными окнами.
...
- `src/components` — компоненты приложения: App, AppHeader, BurgerIngredients, BurgerConstructor, Modal, ModalOverlay, OrderDetails, IngredientDetails
- `src/utils` — функции-обёртки для запросов к API и типы данных
- `src/index.css` — общие стили проекта

Если API курса недоступно, приложение выводит в консоль предупреждение и показывает демо-данные из `src/utils/fallback-ingredients.ts`, чтобы интерфейс оставался работоспособным.
